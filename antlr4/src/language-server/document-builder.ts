...
import { GrammarSpec, ModeSpec, RuleSpec } from "./generated/ast";
...
export class Antlr4DocumentBuilder extends DefaultDocumentBuilder {
  ...
  protected async embedImportedGrammars(
    ...
    const diagnostics: Map<LangiumDocument, Diagnostic[]> = new Map<LangiumDocument, Diagnostic[]>();
    ...
    for (const document of documents) {
      ...
      const rules = toMap(document.parseResult.value.rules.rules, r => r.rule.name);
    ...
    if(diagnostics.size !== 0) {
      ...
      await this.notifyBuildPhase(documents, DocumentState.Validated, cancelToken);
      return  false;
    }
    return true;
